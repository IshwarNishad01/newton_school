...
class Student_model extends MY_Model
{
    ...
    public function findByAdmission($admission_no = null)
    {
        ...
        $this->db->select('student_session.transport_fees,students.vehroute_id,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type ,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,classes.id AS `class_id`,classes.class As class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname, students.middlename, students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode , students.note, students.religion, students.cast, school_houses.house_name,   students.dob ,students.current_address, students.previous_school,
            students.guardian_is,students.parent_id,
    ...
    public function student_all(){
       ...
       // $studentall = $this->db->query('select * from students')->result();
       //  return $studentall;
         $slist = $this->db->query('select students.parent_id,students.admission_no,students.roll_no,students.class_id,students.admission_date,students.section_id,students.firstname,students.middlename,students.lastname,students.rte,students.image,students.mobileno,students.email,students.state,students.city,students.address,students.total_fee,students.concession_percent,students.pincode,students.religion,students.cast,students.dob,students.gender,students.current_address,students.permanent_address,students.category_id,students.route_id,students.school_house_id,students.blood_group,students.vehroute_id,students.hostel_room_id,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name,students.ifsc_code,students.guardian_is,students.father_name,students.father_phone,students.father_occupation,students.mother_name,students.mother_phone,students.mother_occupation,students.guardian_name,students.guardian_relation,students.guardian_phone,students.guardian_occupation,students.guardian_address,students.guardian_email,students.father_pic,students.mother_pic,students.guardian_pic,students.is_active,students.previous_school,students.height,students.weight,students.measurement_date,students.dis_reason,students.note,students.dis_note,students.app_key,students.parent_app_key,students.disable_at,students.created_at,students.updated_at 
        from students join classes on students.class_id=classes.id','left')->result();
         return $slist;
    }

    public function student_datadelete($id){
        $this -> db -> where('id', $id);
        $this -> db -> delete('students');
    }

    public function student_classlist(){
        $classlist = $this->db->query('select * from classes')->result();
        return $classlist;
    }
